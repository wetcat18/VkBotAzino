...
for event in longpoll.listen():
    ...
    if event.type == VkEventType.MESSAGE_NEW:
        ...
        if event.to_me:
            ...
            def fun2 ():
                for i in range(len(status_users)):
                    if str(status_users[i]) == str(event.user_id):
                        ID = i
                        return ID
            ...
            if request == "Начать":
                ...
                if len(status_users) == 1:
                    if str(event.user_id) != status_users[0]:
                        status_users.append(str(event.user_id))
                if len(status_users) == 0:
                    status_users.append(str(event.user_id))
                ...
                if request == "Бюджет" and status1 == 1:
                    status2 = 0
                    status3 = 0
                ...
                if request == "Бюджет" and status11 == 1:
                    status22 = 0
                    status33 = 0
                    write_msg(event.user_id, users_money[fun()])
                elif request == "Загадать" and status11 == 1:
                    status22 = 1
                    status33 = 0
                    write_msg(event.user_id, "Сделайте ставку, ваш бюджет:")
                    write_msg(event.user_id, users_money[fun()])
                    ...
                    status33 = 0
                        ...
                        #Напиши чтобы в случае выхода из игры пользователь удалялся из статус_юзерс лист
                        #Напиши действия если все месте в листе статуса заняты
                        #Реши проблемы вылетов переменных при вводе чисел
